...
class SenateCommitteeFinance extends EventScraper_Abstract 
{
    ...
    protected function scrape()
    {
        ...
        foreach($li[1] as $li_str) {
            if(!preg_match('/<a href=/is',$li_str)) {
                ...
                $events[$i]['couchdb_id'] = (string) $_date_str . ' -  ' .$this->parser_name .' Finance';        
                $events[$i]['datetime'] = (string) $_date_str;
                $events[$i]['end_datetime'] = null;
                $events[$i]['title'] = (string) 'Senate Finance Committee Schedule';
...
$parser = new SenateCommitteeFinance;
$parser->run();
